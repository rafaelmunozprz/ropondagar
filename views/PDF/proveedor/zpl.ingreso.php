...
foreach ($productos as $i => $producto) {
    $sub_total = floatval($producto['cantidad']) * floatval($producto['precio_compra']);
    $total += $sub_total;
    $fila_ac += 30;
    $cuerpo_productos .= '^FO10,' . ($fila_ac) . '^A0,25^FD' . $producto['cantidad'] . '^FS';
    $cuerpo_productos .= '^FO40,' . ($fila_ac) . '^A0,25^FD' . '   x  $ ' . number_format($producto['precio_compra'], 2) . '^FS';
    $cuerpo_productos .= '^FO200,' . ($fila_ac) . '^A0,25^FD' . $producto['data']['nombre'] . '^FS';
    $cuerpo_productos .= '^FO450,' . ($fila_ac) . '^A0,25^FD $ ' . number_format($sub_total, 2) . '^FS';
}

$descuento = $nota_proveedor['descuento'];
...
$cuerpo_cliente = '
	...
	^FO10,570^A0,25^FD' . $proveedor['razon_social'] . '^FS
	^FO10,600^A0,25^FDTeléfono:^FS
	
	^FO10,660^A0,25^FDCorreo:^FS
    ^FO10,690^A0,25^FD' . $proveedor['correo'] . '^FS';
$ll = $fila_ac + 200;
$ZPL_FORMAT = '^XA
	...
	^FO10,760^A0,25^FD      TICKET DE VENTA  ^FS
...
$dir = '../public/';
$documento = 'documentos/zebra/ticket.zpl';

if (is_file($dir . $documento)) {
    unlink($dir . $documento);
}
EscribirNota($ZPL_FORMAT, $dir . $documento);
