...
use App\Email\Templates\TemplateNotaCliente;
...
class Emailtemplate
{
    ...
    function clean_mail($template_mail)
    {
        $temp = explode(" ", $template_mail);
        $temp_n = "";
        foreach ($temp as $text) {
            if ($text != '') $temp_n .= " " . trim($text);
        }
        return $temp_n;
    }
    function notificacion($apartado)
    {
        $cuerpo = '';
        return $cuerpo;
    }
    function reset_password($config)
    {
        $cuerpo = '';
        return $cuerpo;
    }
    function nota_proveedor($nota, $proveedor, $pagos)
    {
        $FUNC = new Funciones();
        $p_body = '';
        $sub_total = 0;
        if ($productos = $nota['productos']) {
            foreach ($productos as $i_prod => $producto) {
                $sub_prod = (floatval($producto['cantidad']) * floatval($producto['precio_compra']));
                $sub_total += $sub_prod;
                $p_body .= '
                    <tr class="fila_tabla" style="padding:0; margin:0;">
                        ...
                        <td style="width:15%;"                   >$ ' . number_format($producto['precio_compra'], 2) . '</td>
        ...
        $template = '
            <div style="min-width: 800px; max-width: 1200px; margin: 0 auto; padding: 15px; font-size:1.3rem; font-family: \'Courier New\', Courier, monospace; text-align:justify;">
                ...
                <p class="MsoNormal"><b>HOLA QUE TAL ' . $proveedor['razon_social'] . ' TE ENVIÓ LOS DATOS DE LA NOTA
                    ANEXANDO LOS PAGOS EN RESUMEN, SI TIENES CUALQUIER DUDA O REQUIERES ACLARACIÓN
                    NO DUDES EN CONTACTARNOS</b></p>
                <h3>Nota de ingreso de productos proveedor</h3>
        ...
        ';
        return $this->clean_mail($template);
    }
    function nota_venta_cliente($nota, $cliente, $pagos)
    {
        require_once 'templates/nota_venta.php';
        $template = TemplateNotaCliente::template_nota($nota, $cliente, $pagos);
        return $this->clean_mail($template);
    }
}
