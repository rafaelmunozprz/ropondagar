...
function mostrar_notas(pagina = 1, buscar = "", proveedor = false, estado = false, nota_proveedores = false) {
    let button = $("#paginacion_p").find('.notas_proveedores');
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.response == 'success') {
                ...
                button.parent().show();
                if (!nota_proveedores) nota_proveedores = respuesta;
                else
                    for (const mod_i of respuesta) {
                        nota_proveedores.push(mod_i);
                    };
                ...
                button.off().on('click', function () {
                    ...
                    button.off();
                    setTimeout(() => {
                        mostrar_notas(pagina + 1, buscar, proveedor, estado, nota_proveedores);
                    }, 500);
                });
                ...
                $(".accion_nota").off().on('click', function () {
                    ...
                    for (const nota of nota_proveedores) {
                        if (nota.id_nota_proveedor == id_nota) {
                            nota_p = nota;
                            break;
                        }
                    }
                    ...
                    if (accion == 'pago_nota') {
                        ...
                        modal.find(".impresion-contenido").html(`
                            <div class="row">
                                <div class="col-12 my-3">
                                    <a target="_blank" href="${RUTA}sistema/proveedor/pdf/facturapdf/${id_nota}" class="btn btn-primary btn-border btn-round  btn-block"> <span class="btn-label"> <i class="fas fa-file-pdf"></i></span>Print PDF</a>
                                </div>
                                <div class="col-12 my-3">
                                    <a target="_blank" href="${RUTA}sistema/proveedor/pdf/ticketzpl/${id_nota}" class="btn btn-primary btn-border btn-round  btn-block" data-tipo="facturazpl"> <i class="fas fa-file-pdf ml-3"></i> Print Zebra</a>
                                </div>`);
                        ...
                        modal.find(".opciones-contenido").html(`
                            <div class="row">
                                ...
                                <div class="col-12 my-3">
                                    <a href="${RUTA}sistema/proveedor/notas/${id_nota}">
                                ...
                                <div class="col-12 my-3">
                                    <a href="${RUTA}sistema/proveedor/facturacion/${id_nota}" class="btn btn-dark btn-border btn-round  btn-block text-dark" > <i class="fas fa-clipboard-list mx-3"></i>Facturación</a>
                                </div>
...
function acciones_nota_view(modal, buscar) {
    $("#send_mail").on('click', function () {
        ...
        setTimeout(() => {
            $.ajax({
                type: "POST",
                url: RUTA + "back/proveedores/email",
            ...
            if (result.value === 'eliminar') {
                setTimeout(() => {
                    $.ajax({
                        type: "POST",
                        url: RUTA + "back/proveedores/nota",
                        data: "opcion=eliminar_nota&id_nota=" + button.parent().attr("data-id_nota"),
                        dataType: "JSON",
                        error: function (xhr, status) {
                            console.log(xhr.responseText);
                            button.html(text).removeAttr("disabled").removeClass('disabled');
